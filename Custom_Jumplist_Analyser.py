...
if int(User_selection) == int(Counter_user):
    ...
    LECMD_command = subprocess.check_output('.\\JLECmd.exe -d %s:\\Users\\%s\\AppData\\Roaming\\Microsoft\\Windows\\Recent\\CustomDestinations --csv .\\Export\\%s' %(Drive_letter,input_command,input_command), shell=True)
    print ("\n\n===============================================================================\n\n" )
    ...
    Mac_check('.\\Export\\%s\\%s'%(input_command,csv_get[0]))
    print ("---------------Show Connections Detected in jump list--------------------------------")
    print( Fore.BLUE + "After Watch the table Press Ctrl+C to go to the next table :" + Fore.RESET)
